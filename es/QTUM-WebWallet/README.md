...
https://Simplechainwallet.org

No hay otra URL para la billetera web de Simplechain, por favor, tengan cuidado y verifiquen el candado verde que dice "Secure"  en el cual pueden corroborar el certificado de seguridad del sitio:
...
Bienvenidos a la documentacion de usuario para la billetera web de Simplechain, a continuacion: 
...
* [How to receive and send Simplechain coins, sending with Ledger](#receive-and-send-Simplechain-coins)
...
The Simplechain web wallet runs in your browser and connects to the Simplechain network through a full node interface. The web wallet does not store your private keys so you must manage those with a downloaded Key File or seed words. This is completely your responsibility.
...
Welcome to the Simplechain web wallet! As shown below the left panel gives a menu with various options to create or restore a wallet, and other actions. The top of the menu gives seven choices for creating or restoring a wallet. The bottom of the menu gives various operations and configuration choices. The center part of the wallet page presents forms for data entry, viewing and management of the wallet assets.
...
The Simplechain web wallet works with Simplechain standard addresses which begin with a "Q" (legacy), and is not compatible with SegWit (Segregated Witness) addresses that begin with an "M" (p2sh-segwit) or "qc1" (bech32).
...
This choice allows restoring the wallet from a Wallet Input Format (WIF) private key such as from the web wallet, Simplechain Core wallet or Simplechain Electrum wallet. A WIF private key will be 52 characters long and has error checking and encoding (to reduce size) as compared with an original private key, which will be 64 hexadecimal characters.
...
Restore from Mobile Wallet allows restoring a wallet address from a compatible mobile wallet such as the Simplechain mobile wallet, Qbao wallet, or the Simplechain Electrum wallet (if the Simplechain Electrum wallet was initialized to be compatible with the Simplechain mobile wallets).
...
To use your Ledger Nano S or Ledger Blue with Simplechain, you will first need to run the Ledger Manager and install the Simplechain app on your Ledger using these instructions:
https://support.ledgerwallet.com/hc/en-us/articles/115003776913-Install-and-use-Simplechain-Simplechain-

![13. Simplechain App EN](https://i.imgur.com/th4IymV.jpg)

On the web wallet select the menu option **` Restore from Ledger `**, plugin your Ledger, and on the Ledger enter the PIN code and open the Simplechain app, then on the web wallet _Restore from Ledger_ page click the red **` CONNECT `** button:
...
# Receive and Send Simplechain Coins
...
Provided you have connected the Ledger hardware wallet, entered the PIN code an selected the Simplechain App, on the Ledger display you will see scrolling details of the transaction so you can confirm the address and amount:

![L4 Address and amount](https://i.imgur.com/K8aYMas.jpg)

On the Ledger, press the right button above the check mark on the display to confirm output #1, which is the main transaction, in this case sending 2.0 Simplechain. You will also need to press this button again to confirm output #2, which is sending the change back to your wallet, and press the button a third time to confirm the overall transaction:
...
To send QRC20 tokens you must have sufficient Simplechain coins in the address tied to that token. The web wallet default fee for sending tokens is 0.00000040 gas price x 250,000 gas = 0.1 Simplechain plus the default transaction fee of 0.01 Simplechain, for a total fee of 0.11 Simplechain. You can use these default values unless you understand how to set lower values, but don't worry, any excess gas will be refunded as a mined amount (the mined amount must mature for 500 blocks before it can be sent or used for gas/fees).
...
The web wallet will has built-in capability for popular QRC20 tokens, and you can add additional tokens by entering the token's smart contract information. For example, if you want to add the XYZ token, find that token on the Explorer Simplechain.info, copy the contract _Address Hash_ field: 

![26. Copy Address Hash EN](https://i.imgur.com/fW00puB.jpg)

On the web wallet select the menu option **` Send `**, click the drop down next to "Simplechain", scroll to the bottom of the Coin/Token list and click the **` More... `** button:
...
Leave _Gas Price_, _Gas Limit_ and _Fee_ set as is, unless you understand how to make changes. If _Gas Limit_ is set too low, the contract creation transaction will run out of gas and fail. For the default settings to the total fee for the token creation contract will be 2,500,000 x 0.00000040 + 0.01 = 1.01 Simplechain, so the wallet must have at least this amount and any excess gas will be refunded.
...
Note that contracts are referenced using their address hash which is a 40-character hexadecimal address and just another way to represent a Simplechain “Q” address.

From this point, you can add the new token to your wallet as shown above [Adding a QRC20 Token](https://github.com/Simplechainproject/documents/tree/master/en/Simplechain-WebWallet-usage#adding-a-qrc20-token).

### Create Contract

**` Create Contract `** allows publishing of any kind of smart contract. To create the contract, start by writing Solidity code and compile to bytecode using a web IDE (Integrated Development Environment) like [qmix](https://qmix.blockchainspaceman.com/) or [remix](http://remix.ethereum.org) or a command line tool like [solar](https://github.com/Simplechainproject/solar).
...
Calling the contract reads state variables in the local copy of the blockchain (in the Simplechain State database) for the web wallet’s server node (without the need for a transaction using gas) and gives an immediate result since there is no need to wait for the next block to publish. For contract calls enter the contract address and the ABI interface.
