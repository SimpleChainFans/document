...
La billetera de Simplechain Core tiene una rica lista comandos, los cuales otorgan un control avanzado y completo de la billetera y transacciones de blockchain. Hay dos grupos de comandos que pueden ser usados con las billeteras de Simplechain Core:
...
Este proceso manual se enfoca en los comandos de consola, los cuales se ingresan en una billetera que esta funcionando y pueden ser enviados usando RPC (Remote Procedure Calls) o en la linea de comandos a la billetera de servidor de Simplechaind o ingresados en la linea de comando al servidor Simplechaind o en la billetera de escritorio con interfaz grafica **Simplechain-qt** por medio de la ventana de depuracion de consola (ver Imagen 1).

![2019-1 Mac Debug Window with Prompt](https://i.imgur.com/K0uCLVr.jpg)
Imagen 1. La Consola de Simplechain-qt
...
Para la billetera servidor Simplechaind, los comandos de consola se ingresan utilizando la interfaz de aplicacion Simplechain-cli (ver Imagen 2).
...
Los Comandos de Consola se le ingresan a una billetera activa de Simplechain Core y provee informacion adicional y control. Estos comandos son necesarios para operar la billetera de servidor Simplechaind, ya que no usa interfaz grafica.
...
See the startup commands on the Simplechain-qt wallet with Help ñ Command line options:

![2019-4 Startup Commands Win](https://i.imgur.com/CryGF13.jpg)
Figure 4. Startup commands

and on the command line itself with ìSimplechaind -?î:
...
Simplechain-qt returns ìnullî after each (see Figure 6).
...
Launch v 0.16 Simplechaind with `-deprecatedrpc=addwitnessaddress` to run the command:
...
The destination can be a filename or path with a filename. The wallet must be fully decrypted (not just for staking only) for this command to work. Simplechain-qt will returns ìnullî, Simplechaind returns nothing. On Windows:
...
Clear all banned nodes IPs. Simplechain-qt returns ìnullî, Simplechaind gives no response.
...
Disconnects a peer (node) using either the IP address or node number. Simplechain-qt will return ìnullî, Simplechaind will return nothing.
...
The difference between `echo` and `echojson` is that `echojson` has argument conversion enabled in the client-side table in Simplechain-cli and the GUI. There is no server-side difference. Here we echo the parameters used to setup a multisig address.
...
Mine up to nblocks immediately to a specified address. Used with the regression test "regtest" private test blockchain. Here we generate 10 genesis blocks on regtest with 20,000 Test Simplechain each, plus an initial block, and `listaddressgroupings` shows the result:
...
DEPRECATED. Returns the current Simplechain address for an account. Use the null account "" for the default address.
...
Get the balance in Simplechain for a wallet.
...
Returns information about the blockchain. Here ìblocksî equals ìheadersî, so this wallet is synced to the latest blocks. ìmoneysupplyî gives the total Simplechain created (genesis blocks + all block rewards). ìsize_on_diskî gives the local storage size for blocks.
...
Hidden command. DEPERCATED. Command line interfaces (but not the Simplechain-qt GUI wallet) may invoke this command using `-getinfo`. The wallet below has a computer clock in sync with Simplechain network time (timeoffset = 0), has 8 peer connections, and is unlocked for a long time (ìunlocked untilî is Unix epoch time in seconds).
...
DEPRECATED. Returns a network hash value related to block mining difficulty, which is not relevant to Simplechain Proof of Stake, for which the network hashes per second is the total number of nodes divided by 16 seconds.
...
Get a new receiving address, with types ìlegacyî, ìp2sh-segwitî or ìbech32î. Simplechain Mainnet legacy addresses start with a ìQî, and SegWit addresses will start with an "M" for p2sh-segwit and "qc1" for bech32. Simplechain Testnet legacy addresses start with a ìqî, and SegWit addresses will start with an "m" for p2sh-segwit and "tq1" for bech32. Here we get a new Mainnet SegWit bech32 address:
...
* ìweightî gives the wallet weight in Satoshis, move the decimal point eight digits to the left to give these weights in units, here wallet weight is 148 Simplechain and the network weight is 12.23 million
...
Returns subsidy (block reward) for the specified block height. Here we see the first block which will have a block reward of 2.0 Simplechain.
...
Get the unconfirmed balance for the wallet, which is the amount in transactions received by the wallet that havenít yet been published in blocks. Here the wallet has received 7.0 Simplechain that haven't been confirmed in a block:
...
* "balance" ñ balance in Simplechain
...
* "paytxfee" ñ the transaction fee in Simplechain per 1,000 bytes
...
Adds a 34-character Simplechain address or 66 hex character public key address that can be watched as if it were in your wallet but cannot be used to spend. The wallet will rescan after entering this command and should be backed up after adding addresses. Simplechain-qt returns ìnullî and displays the ìWatch-onlyî balance, Simplechaind returns nothing.
...
Adds a WIF private key to your wallet, for example as returned by dumpprivkey or from another Simplechain wallet. The wallet must be unlocked and requires a new wallet backup afterwards. The wallet will rescan for a few minutes to add any balance from the new address, and return ìnullî if successful.
...
Adds a public key that can be watched as if it were in your wallet but cannot be used to spend. The public key is 66 characters hex, and can be obtained using the `validateaddress` command. After entering this command, the wallet will rescan for a few minutes, Simplechain-qt returns ìnullî and Simplechaind returns nothing. The wallet should be backed up after importing a public key.
...
Refills the key pool with new private keys, with a default size of 100. The normal size of the keypool is 1,000 and the wallet opportunistically fills the keypool as addresses are used, so this command should use a 1,100 size, etc., to be meaningful. The wallet must be unlocked. Simplechain-qt returns ìnullî, Simplechaind returns nothing.
...
DEPRECATED. Returns information about account names and account balances. Gives the balance in Satoshis (Simplechain x 0.00000001), including the default undefined account.
...
A locked transaction output will not be chosen by automatic coin selection when spending Simplechain.
...
Prioritizes a block with an earlier time for blocks at the same height. Used in hard fork situations. See also `invalidateblock`. Simplechain-qt returns ìnullî if successful, Simplechaind returns nothing if successful.
...
Accepts the transaction into mined blocks at a higher (or lower) priority. A higher fee is not actually paid, the algorithm for selecting transactions into a block (which is trying to maximize fees) considers the transaction as having paid a higher fee. Simplechain-qt and Simplechaind return ìtrueî:
...
Deletes the specified transaction from the wallet. Meant for use with pruned wallets and as a companion to `importprunedfunds`. This will affect wallet balances. Simplechain-cli returns ìnullî.
...
Writes the memory pool to disk in the mempool.dat file. Simplechain-qt returns ìnullî, Simplechaind has no response.
...
DEPRECATED (use `sendtoaddress`). Send an amount to a Simplechain address, requires the wallet to be unlocked. The ìaccountî is for notation, it does not control the source of UTXOs. If successful, returns the transaction ID.
...
Send to multiple addresses, requires the wallet to unlocked. Amounts are floating point in Simplechain. Donít put blank spaces betweeen the parameters. If successful, returns a single transaction ID containing all the outputs. Here we send to two different addresses:
...
Send to multiple addresses, requires the wallet to be unlocked. Amounts are floating point in Simplechain. Can send to duplicate addresses which `sendmany` canít do. If successful, returns a single transaction ID containing all the outputs. Here we send two transactions to the same address:
...
3. "amount"      (numeric or string, optional) The amount in Simplechain to send, default: 0
...
6. "senderaddress" (string, optional) The Simplechain address that will be used as sender
...
DEPRECATED. Assigns and account name to the given address. Simplechain-qt will return ìnullî if successful.
...
Set the transaction fee per kilobyte of the transaction message. Overwrites the paytxfee parameter seen in `getwalletinfo`. The default minimum fee per 1,000 bytes is 0.004 Simplechain. Returns true if successful.
...
Hidden command. Waits for all the asynchronous validation queues (for the blockchain, mempool, etc.) to complete. For use by developers. Simplechain-qt returns ìnullî, Simplechaind returns nothing.
...
Hidden command. Used to monitor when blockchain reloading has reached a given block. For command line interfaces like Simplechaind (not for the Simplechain-qt GUI wallet), the command returns after the given block is loaded. This example waits for Mainnet block 150,000:

```
Simplechain-cli waitforblock ae4699ac0a8f4d1170767167fd3b0639312850ce33dcb55a0afd0f5c1a88406f
...
Hidden command. Waits for (at least) block height and returns the height and blockhash of the current tip (highest block). Returns the current block on timeout or exit. Timeout is given in milliseconds, 0 or default is no timeout. This command only works from the command line (not with the Simplechain-qt GUI wallet) and is used for development.
...
On Mainnet with Simplechaind:

```
Simplechain-cli waitforblockheight 281985
...
Hidden command. Waits for a new block and returns the blockhash and height. Returns the current block on timeout or exit. This command works on command line only (not with the Simplechain-qt GUI wallet) and is used for development. The timeout parameter is given in milliseconds and 0 indicates no timeout.
...
```
Simplechain-cli waitfornewblock
...
Simplechain-cli returns no response, but you can check with `getwalletinfo` for "unlocked_until": 0 and check the padlock icon on Simplechain-qt.
...
The Simplechain-qt wallet will show lock status with the padlock icon and the Console returns ìnullî. Simplechain-cli returns no status but you can check the unlock status with `getwalletinfo` for ìunlocked_untilî: <Unix timestamp>

### walletpassphrasechange "oldpassphrase" "newpassphrase"

Changes the wallet passphrase from "oldpassphrase" to "newpassphrase", Simplechain-qt returns ìnullî, Simplechaind returns nothing.
